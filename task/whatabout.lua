...
--[=[ 需要在配置文件的extraData内提供如下配置项：
    ...
    模块行为：
    - 被点名（@或句首叫小z/zita）、提及、疑似提问时必定触发，其余消息以0.6%概率抽查
    - 点名/提及触发冷却16秒，提问/抽查触发冷却26秒（管理员不受限，超时用蜗牛表情回应）
    - 绝不参与闲聊，只在能猜出词条时发送“猜你想找：#词条1、#词条2”消息
]=]
...
local systemPrompt=STRING.trimIndent[[
    ...
    这个群常有新人提问，一些常见问题能直接从词典里找到答案，如询问概念，或者是能从定义轻松推理出答案的。
    </背景>
