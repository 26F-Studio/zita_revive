...
local extra_tetrio={
    ...
    },
    {
        ...
        text="非T块的Spin不再判为mini（基础攻击=2*行数），但不能连续进行相同消除（以左上角消除文本区为准，不包括B2B和连击），否则会出现一行特殊实心行，在[当前层数+5]次合规消除后转为普通垃圾行（单洞，位置随机）",
    },
    {
        word="DP;2P;duo",
    ...
    },
    {
        ...
        text="在EX基础上，\n击杀奖励从15m降到8m，受击保护释放更快\n不再随时间自动增加高度，而是下坠，每层停留一分钟后开始每秒加0.5%受击倍率\n疲劳事件更早更严苛",
    },
    {
        word="rNH",
    ...
    },
    {
        ...
        text="启用 #困难模式\n非T块不再判spin，禁用攻击，需要挖垃圾行临时启用\n垃圾行为每行随机3~4个灰格，固定4行垃圾，少了立刻出\n初始场地为10行棋盘垃圾行，启用 #垃圾行保护",
    },
    {
        word="rIN",
    ...
    },
    {
        ...
        text="启用 #困难模式\n所有非Spin消除都视为一行，名称改为VOID\n连续相同消除判定更苛刻，两次Spin只要同行数就视为同名，直接罚20行结束游戏\nB2B×4时从4充能开始且B2B加伤提升到+2，且消0行的spin也会触发消行结算加伤加B2B\n开局会收系统10行垃圾，垃圾混乱度增加，启用 #垃圾行保护\n到800m但挖掘垃圾行数小于5时禁止获得经验",
    },
    {
        word="PN",
